  ...
  useEffect(() => {
    axios
      ...
      .then((response) => {
        ...
        const map = {};
        items.forEach((item) => {
          map[item._id] = item.isLiked; // OR item.isLiked if backend sends that
        });
        ...
        const savedMapInit = {};
        items.forEach((item) => {
          savedMapInit[item._id] = item.isSaved;
        });
        setSavedMap(savedMapInit);
      });
  }, []);
      ...
      prev.map((v) =>
        v._id === item._id
          ? {
              ...
              likeCount: Math.max(
                0,
                response.data.like ? v.likeCount + 1 : v.likeCount - 1,
              ),
            }
          : v,
      ),
        ...
        prev.map((v) =>
          v._id === item._id
            ? {
                ...
                saveCount: Math.max(
                  0,
                  res.data.save ? v.saveCount + 1 : v.saveCount - 1,
                ),
              }
            : v,
        ),
              ...
              <div className="actions">
                ...
                <div className="action-item">
                  ...
                  >
                    <svg
                      viewBox="0 0 24 24"
                      fill={
                        likedMap[item._id] === true ? "#ff3b5c" : "currentColor"
                      }
                    >
                ...
                <div className="action-item">
                  <button
                    ...
                    className={`icon-btn ${savedMap[item._id] ? "saved" : ""}`}
                  >
                    <svg
                      xmlns="http://www.w3.org/2000/svg"
                      viewBox="0 0 24 24"
